...
/*   Updated: 2025/08/01 14:37:21 by rhafidi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int handle_builtins(t_tree *root, t_data *data, int *exit_status)
{
    ...
    if (!ft_strcmp(root->command[0], "echo"))
        exit_code = ft_echo(root->command, data->env, exit_status);
        ...
        exit_code = ft_export(root->command,data->env, data->exported, exit_status);
